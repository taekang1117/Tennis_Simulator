...
def scrape_serve_tab(formatted_name, player_name):
    ...
    for row in table.find_all('tr')[1:]:
        ...
        if len(cols) < max(indices.values()) + 1:
            continue
        entry = {col: cols[idx].get_text(strip=True) for col, idx in indices.items()}
        data.append(entry)
...
def scrape_full_stats(player_name):
    ...
    if df_serve is None or df_serve.empty:
        return None

    df_serve.replace('', pd.NA, inplace=True)
    df_serve.dropna(inplace=True)
    return df_serve
