...
'''
def simulate_set(win_prob):
    """Simulate a set following ATP/WTA rules: tiebreak at 6–6, no scores beyond 7–6."""
    a_games, b_games = 0, 0
    ...
    while True:
        ...
        # Early win condition: 6 games with 2-game margin, and no 6–6
        if (a_games == 6 and b_games <= 4) or (b_games == 6 and a_games <= 4):
            break
        if (a_games == 7 and b_games == 5) or (b_games == 7 and a_games == 5):
            break  # This should never happen now, but guards against invalid values
...
def simulate_match_scoreline(win_prob):
    ...
    while a_sets < 3 and b_sets < 3:
        total_games_in_set, a_won = simulate_set(win_prob)
        
        # Reconstruct the actual set score based on winner and total games
        # For simulation purposes, we don't simulate exact (a, b), but we know it must be 6–4, 6–3, ..., or 7–6
        # To simulate full (a, b) detail, you can modify `simulate_set()` to return (a_games, b_games)

        # For now, we use placeholder scoring:
        if a_won:
            a_sets += 1
            if total_games_in_set == 13:
                set_scores.append((7,6))  # tiebreak win
            else:
                set_scores.append((6, total_games_in_set - 6))
        else:
            b_sets += 1
            if total_games_in_set == 13:
                set_scores.append((6,7))  # tiebreak loss
            else:
                set_scores.append((total_games_in_set - 6, 6))
...
model = DecisionTreeModel(max_depth=10000)
