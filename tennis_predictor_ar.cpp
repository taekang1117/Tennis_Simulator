...
vector<MatchData> generate_training_data(int num_samples = 1000) {
    ...
    for (int i = 0; i < num_samples; i++) {
        ...
        // Calculate True Win rates
        normal_distribution<double> tw_dist(65.0, 15.0);
        playerA.TW_percent = max(30.0, min(90.0, tw_dist(generator)));
        playerB.TW_percent = max(30.0, min(90.0, tw_dist(generator)));
        ...
        double ARA = calculate_AR(playerA);
        double ARB = calculate_AR(playerB);
        
        // Apply AR to get adjusted Wsp scores
        double AdjWspA = WspA * ARA;
        double AdjWspB = WspB * ARB;
        
        double CSA = AdjWspA - AdjWspB;
...
int main(int argc, char* argv[]) {
    ...
    if (use_classic) {
        ...
        cout << "--- Classic Formula Prediction " << (use_ar ? "with AR" : "") << " ---\n";
    ...
    if (use_ml) {
        cout << "--- Machine Learning Prediction " << (use_ar ? "with AR" : "") << " ---\n";
        ...
        // Apply AR if enabled
        if (use_ar) {
            // Adjust the ML prediction using global AR coefficient
            double logit = log(ml_winrateA / (1 - ml_winrateA)); // Convert probability to logit
            logit *= global_ar_coefficient; // Apply AR coefficient
            ml_winrateA = 1.0 / (1.0 + exp(-logit)); // Convert back to probability
        }
